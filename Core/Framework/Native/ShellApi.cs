...
namespace Core.Framework.Native
{
    public static class ShellApi
    {
        ...
        public static void GetTaskBarInfo(out ABE taskBarEdge, out int height, out bool autoHide)
        {
            APPBARDATA abd = new APPBARDATA();

            height = 0;
            taskBarEdge = ABE._BOTTOM;
            autoHide = false;

            #region TaskBarEdge & Height
            uint ret = Shell32.SHAppBarMessage((int)ABM._GETTASKBARPOS, ref abd);
            taskBarEdge = (ABE)abd.uEdge;
            switch (taskBarEdge)
            {
                case ABE._BOTTOM:
            ...
            abd = new APPBARDATA();
            uint uState = Shell32.SHAppBarMessage((int)ABM._GETSTATE, ref abd);
            switch (uState)
            {
                case (int)ABS._ALWAYSONTOP:
                    autoHide = false;
                    break;
                case (int)ABS._AUTOHIDE:
                    autoHide = true;
                    break;
                case (int)ABS._AUTOHIDEANDONTOP:
                    autoHide = true;
                    break;
                case (int)ABS._MANUAL:
                    autoHide = false;
                    break;
            }
            #endregion
